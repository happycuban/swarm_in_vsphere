...
resource "vsphere_virtual_machine" "server" {
  ...
  name = "${var.server_name_prefix}-0${count.index + 1}"
  folder  = format("%s/%s", var.vm_folder_root, var.vm_type)

  num_cpus = var.server_num_cpus
  cpu_hot_add_enabled    = var.server_cpu_hot_add_enabled
  cpu_hot_remove_enabled = var.server_cpu_hot_remove_enabled
  memory   = var.server_memory*1024
  memory_hot_add_enabled = var.server_memory_hot_add_enabled
  ...
  clone {
    ...
    customize {
      linux_options {
        host_name = "${var.server_name_prefix}-0${count.index + 1}"
  ...
  provisioner "remote-exec" {
    ...
    connection {
     type = "ssh"
      ...
      host = "${var.server_name_prefix}-0${count.index + 1}"
      user = "sysadmin"
