...
data "vsphere_compute_cluster" "compute_cluster_template" {
  name          = var.vsphere_compute_cluster_template
...
data "vsphere_folder" "folder" {
  count      = var.vm_folder_root != null ? 1 : 0
  path       = "/${data.vsphere_datacenter.dc.name}/vm/${var.vm_folder_root}"
}